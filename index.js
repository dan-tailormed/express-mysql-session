...
module.exports = function(session, sessionKeyName) {
  var constructorArgs;
  ...
  var MySQLStore = function(options, connection, cb) {
    ...
    if (options.sessionKeyName) {
      this.sessionKeyName = options.sessionKeyName;
      delete options.sessionKeyName;
    }
    ...
    this.options.schema = _.defaults(
      this.options.schema,
      defaultOptions.schema
    );
    this.options.schema.columnNames = _.defaults(
      this.options.schema.columnNames,
      defaultOptions.schema.columnNames
    );

    if (this.options.debug) {
      deprecate(
        "The 'debug' option has been removed. This module now uses the debug module to output logs and error messages. Run your app with `DEBUG=express-mysql-session* node your-app.js` to have all logs and errors outputted to the console."
      );
    }
  ...
  MySQLStore.prototype.length = function(cb) {
    ...
    var params = [this.options.schema.tableName];
  ...
  MySQLStore.prototype.defaults = deprecate.function(function defaults(
    object,
    defaultValues,
    options
  ) {
  ...
  if (constructorArgs) {
    ...
    return new (MySQLStore.bind.apply(
      MySQLStore,
      [undefined /* context */].concat(constructorArgs)
    ))();
  }
